...
def clean_text(text):
    ...
    cleaned_prefix = re.sub(
        r"^\s*\d+[a-z]{2}\s+CONGRESS.*?(?:Session|SESSION)\s+\S+\s+",
        "",
        prefix,
...
def filter_by_length(df, min_src=1500, max_src=6000, min_sum=200, max_sum=1500):
    ...
    print(
        f"Length filter ({min_src}-{max_src} chars source, {min_sum}-{max_sum} chars summary): "
...
def main():
    ...
    combined_df = combined_df.drop_duplicates(subset=["near_dedup_key"]).reset_index(
        drop=True
    )
    combined_df = combined_df.drop(columns=["near_dedup_key"])
    print(
        f"Deduplication: removed {before_dedup - len(combined_df)} duplicates/near-duplicates. Remaining: {len(combined_df)}"
    )

    print("\nStep 4: Quality filtering (dropping invalid/empty/flipped examples)...")
    valid_mask = combined_df.apply(
        lambda row: is_valid_example(row["text"], row["summary"]), axis=1
    )
    ...
    if len(filtered_df) < total_needed:
        raise ValueError(
            f"Not enough examples matching criteria. Needed {total_needed}, got {len(filtered_df)}"
        )
    ...
    sampled_pool["len_bucket"] = pd.qcut(
        sampled_pool["text"].str.len(), q=10, labels=False, duplicates="drop"
    )
    ...
    try:
        ...
        print(
            "Warning: Stratified train/test split failed. Falling back to non-stratified split."
        )
        train_val_df, test_df = train_test_split(
            sampled_pool, test_size=200, random_state=42, stratify=None
        )
    ...
    try:
        ...
        print(
            "Warning: Stratified train/val split failed. Falling back to non-stratified split."
        )
        train_df, val_df = train_test_split(
            train_val_df, test_size=300, random_state=42, stratify=None
        )
    ...
    for name, df in [("Train", train_df), ("Val", val_df), ("Test", test_df)]:
        ...
        print(
            f"  {name} set - Avg source length: {avg_src:.1f} chars, Avg summary length: {avg_sum:.1f} chars"
        )
