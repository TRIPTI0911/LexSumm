...
def main():
    ...
    if not torch.cuda.is_available():
        ...
        print(
            "[-] Skipping training run. This script is fully prepared for Kaggle/Colab GPU runtimes."
        )
        sys.exit(0)
    ...
    try:
        ...
        print(
            "[-] To install Unsloth (specifically for Kaggle/Colab environments with CUDA 12.1+):"
        )
        print(
    ...
    if os.path.exists(train_path) and os.path.exists(val_path):
        ...
        hf_dataset_repo_data = os.getenv(
            "HF_DATASET_REPO", "Tripti0911/billsum-processed"
        )
        # Normalize the repo name to remove any domain/prefix if user copied it from web URL
        hf_dataset_repo_data = re.sub(
            r"^(?:https?://)?(?:huggingface\.co/datasets/)?", "", hf_dataset_repo_data
        )
        print(
            f"[+] Local files not found. Loading from HF Hub dataset registry: {hf_dataset_repo_data}..."
        )
        try:
            ...
            print(
                "[-] Please run 'python3 src/data_prep.py' locally or set a valid HF_DATASET_REPO environment variable."
            )
            sys.exit(1)
